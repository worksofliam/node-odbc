...
class Pool {
   ...
   * @returns 
   */
  async callProcedure(catalog, schema, procedure, params, cb) {
    // determine the parameters passed
    let callback = cb;
    let parameters = params;
    ...
    if (typeof callback !== 'function') {
      return new Promise((resolve, reject) => {
        connection.callProcedure(catalog, schema, procedure, parameters, (error, result) => {
          // after running, close the connection whether error or not
          if (error) {
            reject(error);
          } else {
    ...
    // ...or callback
    return connection.callProcedure(catalog, schema, procedure, parameters, (error, result) => {
      // after running, close the connection whether error or not
      process.nextTick(() => {
        callback(error, result);
      });
